...
<li>select * from EmployeePosition order by Salary desc;</li>
<hr>
...
WHERE (SELECT COUNT(DISTINCT E2.Salary) 
 ...
 SELECT distinct( Salary ) as thirdmaxSalary
FROM EmployeePosition E1 
WHERE (SELECT COUNT(DISTINCT E2.Salary) 
       FROM EmployeePosition E2 
...
WHERE (SELECT COUNT(DISTINCT E2.Salary) 
       ...
       -- inner loop tells how many from e2 are bigger than the current 
-- so when we get count = n specified one means n are greater than that e1 salary 
